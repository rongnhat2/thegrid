 ...
 * opt_backtest.js
 *
 * - Load SOLUSDT-1h.csv (timestamp,open,high,low,close,volume)
 ...
 *
 * Mode: CONSERVATIVE RISK MANAGEMENT (Target: 10-12% ROI with lower drawdown)
 * - Lower position sizes (5-20 USDT per grid)
 * - Tighter spacing (1-1.8%)
 * - Conservative take-profit (2-3%)
 * - Safety filters enabled (sentiment, netflow)
 *
 * Usage:
 *   npm i csv-parse
...
function runBacktest(params, showProgress = false) {
    ...
    const BASE_TP = 0.1; // 2.5% base take-profit (conservative: 2-3% range for safer, consistent returns)
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        // CONSERVATIVE MODE: Enable filters to reduce risk and protect capital
        // Pause grid during extreme sentiment to avoid catching falling knives
        const sIdx = sim.sentiment_index[i] || 0;
        const lsr = sim.long_short_ratio[i] || 1.0;
        if (sIdx >= 1.5 && lsr > 2.0) {
            gridEnabled = false; // Pause grid during extreme bullish sentiment (potential reversal)
        } else if (sIdx <= -1.0) {
            spacing = BASE_SPACING * 0.85; // Reduce spacing slightly during negative sentiment
        }

        // ========== TIER 1: ON-CHAIN FUND FLOW FILTER ==========
        // CONSERVATIVE MODE: Adjust spacing based on netflow to reduce risk
        // Tighten spacing during strong outflows, widen during inflows
        const nf = sim.netflow_z[i] || 0;
        if (nf > 3 * NETFLOW_SENS) {
            spacing = BASE_SPACING * 0.9; // Slightly tighter spacing during strong outflows
        } else if (nf < -2 * NETFLOW_SENS) {
            spacing = BASE_SPACING * 1.1; // Slightly wider spacing during strong inflows
        }

        // ========== TIER 2: VOLATILITY FILTER ==========
        // CONSERVATIVE MODE: Adaptive take-profit based on volatility (2-3% range)
        // Calculation: ATR(14) ÷ Current Price = Real-time Volatility %
        // Rules (CONSERVATIVE):
        // - Low volatility → 2% take-profit (safer, more frequent exits)
        // - High volatility → 3% take-profit (capture moves while staying conservative)
        // - Default: 2.5% take-profit
        // This balances profit capture with risk management
        const vol = indicators.volPct[i] || 0;
        if (vol < 0.01) {
            tp = 0.02; // 2% - Low volatility, conservative and achievable
        } else if (vol > 0.03) {
            tp = 0.03; // 3% - High volatility, capture moves while staying safe
        } else {
            tp = BASE_TP; // Default TP 2.5% for conservative, consistent returns
        }

        // CLAMP TP: 1% ≤ TP ≤ 5% (conservative range: 2-3% focus)
        tp = Math.max(0.01, Math.min(0.05, tp));
        ...
        if (rsiVal !== null && bbVal !== null) {
            ...
            if (rsiVal < 25 && close < bbVal.lower) {
                const usdtPos = GRID_UNIT_USDT * GRID0_MULT; // Conservative position size (0.5×-1× standard)
...
// ---------- SEARCH SPACE ----------
// CONSERVATIVE RISK MANAGEMENT CONFIG (Target: 10-12% ROI with lower drawdown)
const SEARCH_RANGES = {
    GRID_UNIT_USDT: [5, 10, 15, 20], // Reduced from 20-50→5-20 USDT per grid for lower exposure
    BASE_SPACING: [0.01, 0.012, 0.015, 0.018], // Tighter spacing 1-1.8% for more frequent, safer trades
    GRID0_MULT: [0.5, 0.75, 1.0], // Reduced to 0.5×-1× standard grid size for conservative accumulation
    NETFLOW_SENS: [0.5, 0.7, 1.0], // Slightly reduced sensitivity options
};
...
if (best) {
    ...
    fs.writeFileSync("best_equity1.csv", csv);
    ...
    fs.writeFileSync("best_summary_1.json", JSON.stringify(summary, null, 2));
