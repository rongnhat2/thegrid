...
function runBacktest(params, showProgress = false) {
    ...
    const BASE_TP = 0.5; // 50% base take-profit (tăng lên 50% - rủi ro rất cao)
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        // Rules:
        // - Volatility < 1% → Take-profit 40% (vẫn đảm bảo lợi nhuận cao)
        // - Volatility > 3% → Take-profit 60% (capture more in volatile markets)
        // - Default: TP 50% (tăng lên 50% - rủi ro rất cao)
        // Note: Minimum TP must be > 0.2% (round trip fees) to be profitable
        // ⚠️ CẢNH BÁO: TP 50% rất cao, xác suất đạt TP thấp, nhiều lệnh có thể không bao giờ chốt lời
        const vol = indicators.volPct[i] || 0;
        if (vol < 0.01) {
            tp = 0.4; // 40% - Low volatility, vẫn đảm bảo lợi nhuận cao
        } else if (vol > 0.03) {
            tp = 0.6; // 60% - High volatility, capture more in volatile markets
        } else {
            tp = BASE_TP; // Default TP 50% (tăng lên 50% - rủi ro rất cao)
        }

        // CLAMP TP: 0.5% ≤ TP ≤ 100% (cho phép TP rất cao để đạt lợi nhuận 50%+)
        tp = Math.max(0.005, Math.min(1.0, tp));
